#!/usr/bin/env python3
#
# Entry for https://adventofcode.com/2024/day/8
...
for freq, coords in antennas.items():
    for a, b in itertools.combinations(coords, 2):
        antinode_a = a + (a - b)
        if antinode_a.in_bounds(rows, cols):
            resonances.add(antinode_a)
        antinode_b = b + (b - a)
        if antinode_b.in_bounds(rows, cols):
            resonances.add(antinode_b)
